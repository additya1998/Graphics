...
void move_up(){
	boat.position.z = boat.position.z -  0.15 * cos(boat.rotation * M_PI / 180);
	boat.position.x = boat.position.x - 0.15 * sin(boat.rotation * M_PI / 180);	
}
...
void tick_elements() {
	// printf("X %f %f\n", boat.getBoundingBox().low_x, boat.getBoundingBox().high_x);
	// printf("Y %f %f\n", boat.getBoundingBox().low_y, boat.getBoundingBox().high_y);
	// printf("Z %f %f\n", boat.getBoundingBox().low_z, boat.getBoundingBox().high_z);
	boat.tick();
...
void set_camera(int idx){
	...
	// follow boat
	if(idx == 0){
		camera[0].eye = glm::vec3 (boat.position.x + 2 * sin(boat.rotation * M_PI / 180.0f), 1, boat.position.z + 2 * cos(boat.rotation * M_PI / 180.0f));
	...
	// top view
		...
		camera[1].target = glm::vec3(boat.position.x, 0, boat.position.z + 0.5);
