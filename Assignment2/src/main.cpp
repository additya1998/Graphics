...
void tick_input(GLFWwindow *window) {
    ...
    int view = glfwGetKey(window, GLFW_KEY_RIGHT);
    if (left) {
        // Do something
    }
}
...
void initGL(GLFWwindow *window, int width, int height) {
    ...
    // set camera
    for(int i=0; i<5; ++i){
        camera[i].eye = glm::vec3 (0, 0, 5);
        camera[i].target = glm::vec3 (0, 0, 0);
        camera[i].up = glm::vec3 (0, 1, 0);
    }
...
void set_camera(int idx){
    ...
    // follow
    if(idx == 0){
        camera[0].eye = glm::vec3 (5*cos(camera_rotation_angle*M_PI/180.0f), 0, 5*sin(camera_rotation_angle*M_PI/180.0f));
        camera[0].target = glm::vec3 (ball1.position.x, ball1.position.y, ball1.position.z);
